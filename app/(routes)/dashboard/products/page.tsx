		...
		<div className="container">
			...
			<div className="w-[90%] mx-auto grid grid-cols-1 lg:grid-cols-7 md:grid-cols-6 sm:grid-cols-4 gap-3 mb-5">
				{productsList.map(product =>
					<div key={product.id} className="shadow-xl p-2 rounded-[7px]">
						<Image src={product.photo} alt="Product_image" priority width={250} height={250}/>
						<div className="my-1 flex justify-between items-center">
							<span className="overflow-hidden h-5">{product.name}</span>
							<span className="text-sm text-textSecondary">{product.brand}</span>
						</div>
