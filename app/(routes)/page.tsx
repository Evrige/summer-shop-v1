          ...
          {!searchParams && user &&
            <div>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-7 lg:grid-cols-4 gap-3">
                {userHistoryList?.data?.map(product => <UserProductsItem product={product} key={product.id}/>)}
              </div>
              <h2 className="mt-1">Спеціяльно для вас:</h2>
              <div className="grid grid-cols-1 xl:grid-cols-7 lg:grid-cols-4 gap-3">
                {userTopProductList?.data?.map(product => <UserProductsItem product={product} key={product.id}/>)}
              </div>
            </div>
          }
          <h2 className="mt-1">Перелік товарів:</h2>
          <UserProductsList/>
        </div>
      </section>
    </main>
